...
### Return an executor a.k.a a model that we can feed the input to
### and the context (gpu or cpu)
def torch2executor(torch_module: torch.nn.Module, torch_inputs: Tuple[torch.Tensor, ...], target):
    # Names to load pre-compiled TVM module, graph, params
    prefix = f"mobilenet_tsm_tvm_{target}"
    lib_fname = f'{prefix}.tar'
    graph_fname = f'{prefix}.json'
    params_fname = f'{prefix}.params'
    
    # Either load the above, or create new one using the function torch2tvm_module
