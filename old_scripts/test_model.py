...
from ops.dataset import TSNDataSet, PhonePackagingDataset
...
parser.add_argument('--dir_save_result', type=str, default='eval.txt')
parser.add_argument('--dir_save_per_vid', type=str)
...
for this_weights, this_test_segments in zip(weights_list, test_segments_list):
    ...
    num_class, args.train_list, eval_list, root_path, prefix = dataset_config.return_dataset(args.dataset, modality, args.categories_file)
    ...
    if args.dataset == "Assembly101": 
        load_dataset = TSNDataSet
    elif args.dataset == "PhonePackaging": 
        load_dataset = PhonePackagingDataset
    data_loader = torch.utils.data.DataLoader(
            load_dataset(root_path, eval_list, num_segments=this_test_segments,
...
for i, data_label_pairs in enumerate(zip(*data_iter_list)):
    with torch.no_grad():
        ...
        # _, test = torch.from_numpy(ensembled_predict).topk(5, 1, True, True)
        # test = test.t()
        # test = test.numpy()
        # F = open(args.dir_save_per_vid, "a")
        # for m in range(test.shape[1]):
        #     for n in range(test.shape[0]):
        #         F.write(str(test[n,m])+' ')
        #     F.write('\n')
        # F.close()
        ...
        if i % 1 == 0:
            print('video {} done, total {}/{}, average {:.3f} sec/video, '
                  'moving Prec@1 {:.3f} Prec@5 {:.3f}'.format((i+1) * args.batch_size, (i+1) * args.batch_size, total_num,
                                                              float(cnt_time) / (i+1) / args.batch_size, top1.avg, top5.avg))

result = 'video {} done, total {}/{}, average {:.3f} sec/video, ''moving Prec@1 {:.3f} Prec@5 {:.3f}'.format(i * args.batch_size, i * args.batch_size, total_num,float(cnt_time) / (i+1) / args.batch_size, top1.avg, top5.avg)
...
precision = np.sum(np.array(video_labels)==np.array(video_pred))*100.0/len(video_pred)
F = open(args.dir_save_result, "a")
F.write(args.video_id + ": "+result+" overall: " +str(precision)+'\n')
F.close()
